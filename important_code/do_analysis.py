...
class analyze_data():
    def __init__(self, analysis, dataframe, analysis_type):
        ...
        if analysis_type == 'escape paths': self.quantities_to_analyze = ['speed', 'start time', 'end time', 'path', 'edginess', 'RT']
        ...
        if analysis_type == 'edginess': self.quantities_to_analyze = ['start time', 'end time', 'path', 'edginess','RT', 'prev homings', 'prev movements',
                                                                      'in center', 'movement']
        if analysis_type == 'efficiency': self.quantities_to_analyze = ['start time', 'end time','optimal path length', 'path', 'prev homings', 'edginess',
                                                                        'optimal RT path length','full path length','RT path length', 'prev movements']
    ...
    def main(self, analysis, dataframe, analysis_type):
        ...
        for i, session_name in enumerate(analysis.selected_sessions):
            # get the metadata
            self.get_metadata(session_name, analysis, dataframe)
    ...
    def analyze_escapes(self, coordinates, analysis):
        ...
        # initialize data
        distance_from_shelter, position, angular_speed, speed, distance_map = self.initialize_escape_data(coordinates, analysis)
        ...
        for stim in self.stims_all[self.vid_num]:
            ...
            if arrived_at_shelter.size and RT.size and not ('no shelter' in self.experiment and not 'down' in self.experiment) and arrived_at_shelter[0] > 10:
                ...
                if 'optimal path length' in self.quantities_to_analyze: self.analyze_optimal_path_lengths(distance_map, position, stim, arrived_at_shelter, speed, RT, analysis)
                if 'edginess' in self.quantities_to_analyze: x_edge = self.analyze_edginess(RT_speed, angular_speed, arrived_at_shelter, position, subgoal_speed_trace, stim_idx, threat_idx, analysis)
                ...
                #if 'prev homings' in self.quantities_to_analyze: self.analyze_prev_homings(coordinates, stim, x_edge, analysis)
    ...
    def analyze_traversals(self, coordinates, analysis):
        ...
        edginess = {}
        for traversal_type, traversals in zip(['back traversal', 'front traversal'],[traversals_from_back, traversals_from_front]):
            edginess[traversal_type] = []
            for traversal in traversals:
                trav_edginess = self.compute_edginess(traversal[0]*self.scaling_factor, traversal[1]*self.scaling_factor, traversal_type)
                edginess[traversal_type].append(trav_edginess)

        # add to the analysis dictionary
        analysis.analysis[self.experiment][self.condition]['back traversal'][self.mouse] = [traversals_from_back, traversals_time_back, escapes_from_back, escape_times, edginess['back traversal']]
        analysis.analysis[self.experiment][self.condition]['front traversal'][self.mouse] = [traversals_from_front, traversals_time_front, [], [], edginess['front traversal']]
    ...
    def fill_in_nans(self, analysis):
        ...
        escape_dependent_fields = ['end time', 'RT', 'edginess', 'optimal path length', 'optimal RT path length', 'full path length', 'RT path length',
                                   'prev homings', 'prev movements']
    ...
    def analyze_prev_homings(self, coordinates, stim, x_edge, analysis):
        ...
        analysis.analysis[self.experiment][self.condition]['prev homings'][self.mouse].append([x_SH, y_SH, how_long_to_shelter, SH_time, x_edge, stim_evoked])
    ...
    def initialize_escape_data(self, coordinates, analysis):
        ...
        return distance_from_shelter, position, angular_speed, speed, distance_map
    ...
    def get_metadata(self, session_name, analysis, dataframe):
